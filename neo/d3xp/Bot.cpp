...
void idBotState::EvaluateMapGoals(botFrameState_t& state)
{
    ...
    else
    {
        ...
        if (skill == 0)
        {
            personality.accuracy = 0.2f;
            personality.aggression = 0.2f;
            personality.jumpFrequency = 0.1f;
        }
        else if (skill == 1)
        {
            personality.accuracy = 0.5f;
            personality.aggression = 0.5f;
            personality.jumpFrequency = 0.3f;
        }
        else if (skill == 2)
        {
            personality.accuracy = 0.8f;
            personality.aggression = 0.8f;
            personality.jumpFrequency = 0.6f;
        }
        else
        {
            personality.accuracy = 1.0f;
            personality.aggression = 1.0f;
            personality.jumpFrequency = 1.0f;
        }
    }
    ...
    for (idEntity* ent = gameLocal->spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next())
    {
        ...
        else if (idStr::Icmpn(classname, "item_powerup", 12) == 0 || idStr::Icmpn(classname, "item_megahealth", 15) ==
            0)
        {
...
void idBotState::BotSensoryAcquisition(botFrameState_t& state)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        if (currentEnemyVisible)
        {
            ...
            if (!heardEnemy && (viewForward * dirToEnemy < 0.5f))
            {
                // Outside 120-degree FOV cone
...
void idBotState::BotTargetPositionSelection(botFrameState_t& state)
{
    ...
    if (isCTF && friendlyFlag && enemyFlag)
    {
        ...
        idPlayer* enemyCarrier = (enemyCarrierIdx != -1)
                                     ? static_cast<idPlayer*>(gameLocal->entities[enemyCarrierIdx])
                                     : NULL;
            ...
            if (gameLocal->time >= botNextTargetSearchTime || !botHasTargetPos)
            {
                ...
                if (aas && gameLocal->num_entities > 0)
                {
                    for (int attempts = 0; attempts < 10; attempts++)
                    {
                        ...
                        {
                            int targetArea = aas->PointReachableAreaNum(ent->GetPhysics()->GetOrigin(),
                                                                        client->GetPhysics()->GetBounds(),
                                                                        AREA_REACHABLE_WALK);
                            if (targetArea > 0)
                            {
...
void idBotState::BotNavigationPathfinding(botFrameState_t& state)
{
    ...
    if (gameLocal->time >= botNextPathTime || botCachedMoveDir == vec3_origin)
    {
        ...
        if (aas && botHasTargetPos)
        {
            ...
            trace_t directTr;
            gameLocal->GetClip()->Translation(directTr, myOrigin + idVec3(0, 0, 16), botTargetPos + idVec3(0, 0, 16),
                                              client->GetPhysics()->GetClipModel(), mat3_identity, MASK_PLAYERSOLID,
                                              client);
            ...
            else
            {
                ...
                if (myAreaNum > 0 && targetAreaNum > 0)
                {
                    ...
                    if (aas->WalkPathToGoal(path, myAreaNum, org, targetAreaNum, goal, TFL_WALK | TFL_AIR))
                    {
                        ...
                        if (path.type == PATHTYPE_BARRIERJUMP || path.type == PATHTYPE_JUMP || botCachedMoveDir.z >
                            24.0f)
                        {
    ...
    if (botCachedMoveDir == vec3_origin)
    {
        ...
        else
        {
            ...
            trace_t tr;
            gameLocal->GetClip()->Translation(tr, myOrigin + idVec3(0, 0, 16),
                                              myOrigin + idVec3(0, 0, 16) + forwardDir * 80.0f,
                                              client->GetPhysics()->GetClipModel(), mat3_identity, MASK_PLAYERSOLID,
                                              client);

            bool stuck = (client->GetPhysics()->GetLinearVelocity().Length() < 5.0f && gameLocal->time > botWanderTime -
                1500);
            ...
            if (tr.fraction < 1.0f || stuck)
            {
                ...
                gameLocal->GetClip()->Translation(trLeft, myOrigin + idVec3(0, 0, 16),
                                                  myOrigin + idVec3(0, 0, 16) + dirLeft * 80.0f,
                                                  client->GetPhysics()->GetClipModel(), mat3_identity, MASK_PLAYERSOLID,
                                                  client);
                gameLocal->GetClip()->Translation(trRight, myOrigin + idVec3(0, 0, 16),
    ...
    else
    {
        ...
        if (botHasTargetPos && client->GetPhysics()->GetLinearVelocity().Length() < 5.0f)
        {
            ...
            {
                // 20% chance to jump when stuck on a path
                botCachedNeedJump = true;
            }
        }
        ...
        trace_t avoidTr;
        gameLocal->GetClip()->Translation(avoidTr, myOrigin + idVec3(0, 0, 16),
                                          myOrigin + idVec3(0, 0, 16) + forwardDir * 120.0f,
                                          client->GetPhysics()->GetClipModel(), mat3_identity, MASK_PLAYERSOLID,
                                          client);
...
void idBotState::BotAimingAndRotation(usercmd_t& cmd, botFrameState_t& state)
{
    ...
    if (state.enemyVisible && state.nearestEnemy)
    {
        ...
        else if (idStr::Icmp(currentWeaponName, "weapon_plasma") == 0 || idStr::Icmp(currentWeaponName, "weapon_bfg") ==
            0) projSpeed = 800.0f;
        ...
        if (idStr::Icmp(currentWeaponName, "weapon_machinegun") == 0 || idStr::Icmp(
            currentWeaponName, "weapon_chaingun") == 0)
        {
...
void idBotState::BotActionAndEvasion(usercmd_t& cmd, botFrameState_t& state)
{
    ...
    if (state.nearestEnemyDist > 800.0f && (idStr::Icmp(currentWeaponName, "weapon_machinegun") == 0 || idStr::Icmp(
        currentWeaponName, "weapon_chaingun") == 0))
    {
        ...
        if (trForward.fraction < 1.0f)
        {
            ...
            else
            {
                ...
                botWanderYaw = idMath::AngleNormalize360(
                    botWanderYaw + 90.0f + gameLocal->random.CRandomFloat() * 45.0f);
...
void idBotState::BotCombatStrafing(usercmd_t& cmd, botFrameState_t& state)
{
    if (state.enemyVisible && state.nearestEnemy)
    {
        ...
        if (idStr::Icmp(currentWeaponName, "weapon_shotgun") == 0 || idStr::Icmp(
            currentWeaponName, "weapon_doublebarrel") == 0)
        {
            idealDist = 120.0f;
        }
        else if (idStr::Icmp(currentWeaponName, "weapon_rocketlauncher") == 0 || idStr::Icmp(
            currentWeaponName, "weapon_machinegun") == 0)
        {
            idealDist = 600.0f;
        }
        ...
        if (idStr::Icmp(currentWeaponName, "weapon_fists") == 0 && state.nearestEnemyDist > 120.0f) wantsToRetreat =
            true;
        ...
        if (moveDir.Normalize() > 0.1f)
        {
            ...
            if (!bypass)
            {
                ...
                if (tr.fraction >= 1.0f || (start.z - tr.endpos.z) > (18.0f + 40.0f))
                {
                    ...
                    {
                        botWanderYaw = idMath::AngleNormalize360(
                            botWanderYaw + 180.0f + gameLocal->random.CRandomFloat() * 45.0f);
