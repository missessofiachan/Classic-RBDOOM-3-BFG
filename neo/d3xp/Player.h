...
public:
  ...
  void
  Restore(idRestoreGame *savefile); // unarchives object from save game file
  ...
  bool Give(idPlayer *owner, const idDict &spawnArgs, const char *statname,
            const char *value, idPredictedValue<int> *idealWeapon,
            bool updateHud, unsigned int giveFlags);
  void Drop(const idDict &spawnArgs, const char *weapon_classname,
            int weapon_index);
  ammo_t AmmoIndexForAmmoClass(const char *ammo_classname) const;
  int MaxAmmoForAmmoClass(const idPlayer *owner,
                          const char *ammo_classname) const;
  int WeaponIndexForAmmoClass(const idDict &spawnArgs,
                              const char *ammo_classname) const;
  ammo_t AmmoIndexForWeaponClass(const char *weapon_classname,
                                 int *ammoRequired);
  ...
  bool CanGive(idPlayer *owner, const idDict &spawnArgs, const char *statname,
               const char *value);
...
public:
  ...
  float independentWeaponPitchAngle; // viewAngles[PITCH} when head tracking is
                                     // active
  ...
  float controllerShakeHighMag[MAX_SHAKE_BUFFER]; // magnitude of the high
                                                  // frequency controller shake
  float controllerShakeLowMag[MAX_SHAKE_BUFFER];  // magnitude of the low
                                                  // frequency controller shake
  int controllerShakeHighTime[MAX_SHAKE_BUFFER];  // time the controller shake
                                                  // ends for high frequency.
  int controllerShakeLowTime[MAX_SHAKE_BUFFER];   // time the controller shake
                                                  // ends for low frequency.
...
public:
  ...
  // save games
  void Save(idSaveGame * savefile) const; // archives object for save game file
  void Restore(idRestoreGame *
               savefile); // unarchives object from save game file
  ...
  virtual void GetAIAimTargets(const idVec3 &lastSightPos, idVec3 &headPos,
                               idVec3 &chestPos);
  virtual void DamageFeedback(idEntity * victim, idEntity * inflictor,
                              int &damage);
  void CalcDamagePoints(idEntity * inflictor, idEntity * attacker,
                        const idDict *damageDef, const float damageScale,
                        const int location, int *health, int *armor);
  ...
  void ServerDealDamage(int damage, idEntity &inflictor, idEntity &attacker,
                        const idVec3 &dir, const char *damageDefName,
                        const int location); // Actually updates the player's
  ...
  virtual void Killed(idEntity * inflictor, idEntity * attacker, int damage,
  ...
  void GiveObjective(const char *title, const char *text,
                     const idMaterial *screenshot);
  ...
  void SetInfluenceView(const char *mtr, const char *skinname, float radius,
                        idEntity *ent);
  ...
  virtual void ClientThink(const int curTime, const float fraction,
                           const bool predict);
...
private:
  ...
  idList<aasLocation_t, TAG_IDLIB_LIST_PLAYER>
      aasLocation; // for AI tracking the player
  ...
  int tweap; // GK: In order to keep track on whenever we want to cycle through
             // the weapons
  ...
  static const int NUM_LOGGED_VIEW_ANGLES =
      64; // for weapon turning angle offsets
  idAngles loggedViewAngles
      [NUM_LOGGED_VIEW_ANGLES]; // [gameLocal->framenum&(LOGGED_VIEW_ANGLES-1)]
